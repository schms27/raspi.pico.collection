...
__author__ = "%(__author_name__)s <%(__author_email__)s>"
...
if _copyright_year_latest > _copyright_year_begin:
    _copyright_year_range += "–%(_copyright_year_latest)s"
__copyright__ = (
    ...
    def __init__(self, *args, **kwargs):
        if self.__class__ is EnumException:
            ...
            raise NotImplementedError(
                "%(class_name)s is an abstract base class")
...
class EnumBadKeyError(TypeError, EnumException):
    ...
    def __str__(self):
        return "Enumeration keys must be strings: %(key)r"
...
class EnumValue(object):
    ...
    def __repr__(self):
        return "EnumValue(%(_enumtype)r, %(_index)r, %(_key)r)"
